...
+ (NSString *)dateStringFromeTimeInterval:(NSInteger)timeInterval {
    ...
    //距离当前时间的秒数
    NSInteger createdAt = nowTime - timeInterval;
    //用来接收返回的时间段
    NSString *timeStr = [[NSString alloc] init];
    if (createdAt / 60) {
        if (createdAt / (60 * 60)) {
            if (createdAt / (60 * 60 * 24)) {
                if (createdAt / (60 * 60 * 24 * 28)) {
                    NSString *timeNum = [NSString stringWithFormat:@"%d",createdAt];
                    timeStr = [WHHelper dateStringFromNumberTimer:timeNum];
                } else {
                    timeStr = [NSString stringWithFormat:@"%d天前",createdAt / (60 * 60 * 24)];
                }
            } else {
                timeStr = [NSString stringWithFormat:@"%d小时前",createdAt / (60 * 60)];
            }
        } else {
            timeStr = [NSString stringWithFormat:@"%d分钟前",createdAt / 60];
        }
    } else {
        timeStr = @"刚刚";
    }
